...
public final class ScaleHelper {
    ...
    //更新缩放比例
    private void updateScales(float[] scales) {
        if (scales.length == 0) {
    ...
    private void appendIfNeed() {
        ...
        if (mScales[mScales.length - 1] != 1) {
            mScales = insertElement(true, mScales, 1, 1);
        }
    }
     ...
     * 扩展数组元素
     *
     * @param isAddFormed 是否从头部添加
     * @param target      目标数组
     * @param elements    需要插入的数值
     * @return 扩展后的数组
     */
    private float[] insertElement(boolean isAddFormed, float[] target, float... elements) {
     ...
     * fraction 当前位置（0-1）
     */
    public float getScale(float fraction) {
        float minScale = 1;
        float maxScale = 1;
        float scalePosition;
    ...
    //判断数值是否合法，value 要判断的数值，true 为合法，反之不合法
